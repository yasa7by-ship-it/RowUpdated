...
async function executeTranslations() {
  ...
  try {
    ...
    if (enError) {
      console.log(`   ❌ خطأ في إضافة الترجمة الإنجليزية: ${enError.message}`);
    } else {
    ...
    if (arError) {
      console.log(`   ❌ خطأ في إضافة الترجمة العربية: ${arError.message}`);
    } else {
    ...
    if (!checkError && checkData) {
      ...
      if (checkData.length === 2) {
        ...
        console.log('✅ يجب أن تظهر "جدول أداء الأسهم" بدلاً من "stock_performance_table"');
      } else {
