-- #############################################################################
-- # RPC Function شاملة لتنفيذ جميع التحديثات
-- #############################################################################

BEGIN;

-- إنشاء وظيفة شاملة لتنفيذ جميع التحديثات
CREATE OR REPLACE FUNCTION public.execute_evaluation_system_update()
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
-- استدعاء الوظيفة
SELECT public.execute_evaluation_system_update();
