-- #############################################################################
-- # تحديث وظيفة evaluate_and_save_forecasts لإرجاع JSON مع الإحصائيات
-- # تنفيذ مباشر في Supabase SQL Editor
-- #############################################################################

BEGIN;

-- حذف الوظيفة القديمة أولاً (لأن نوع الإرجاع تغير)
DROP FUNCTION IF EXISTS public.evaluate_and_save_forecasts(date);

-- إنشاء الوظيفة الجديدة
CREATE FUNCTION public.evaluate_and_save_forecasts(p_date_filter date DEFAULT NULL)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
